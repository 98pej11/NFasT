...
public interface NfastRepository extends JpaRepository<Nfast,Long> {
    ...
    //구매 가능한 NFast 리스트 출력
    @Query(value="select nfast_date as nfastDate,count(*) as amount, min(nfast_hope_price) as price from nfast where store_sequence=?1 and nfast_sale_state!=1 group by nfast_date order by nfast_date",nativeQuery = true)
    List<NfastPurchase> findAllByStoreSequenceOrderByNfastDateDesc(long storeSequence);

    //구매할 날짜 nft 상세 정보 출력
    @Query(value="select nfast_date as nfastDate, count(*) as amount, nfast_hope_price as price from nfast where nfast_date=?1 and nfast_sale_state!=1 group by nfast_hope_price order by nfast_hope_price", nativeQuery = true)
...
//    count(case when nfast_sale_state=0 then 1 end) as nfastSaleCount, count(*) as nfastTotalCount
    ...
    @Query(value="select * from nfast where nfast_sale_state!=1 and store_sequence=?1 and nfast_date=?2 and nfast_hope_price=?3 limit ?4", nativeQuery = true)
